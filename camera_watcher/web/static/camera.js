(function () {
  ...
  const CAMERA_ID = document.body.dataset.cameraId;
  const API = (path) => "/api/cameras/" + encodeURIComponent(CAMERA_ID) + path;
  ...
  // ---------- Delete camera ----------
  document.getElementById("delete-camera").addEventListener("click", () => {
    const deleteData = document.getElementById("delete-data-checkbox").checked;
    const warning = deleteData
      ? `Remove "${CAMERA_ID}" and permanently delete its recordings/cache from disk?`
      : `Remove "${CAMERA_ID}"? Its recordings/cache stay on disk unless you also check "delete data".`;
    if (!window.confirm(warning)) return;
    fetch(API("") + (deleteData ? "?delete_data=true" : ""), { method: "DELETE" })
      .then((r) => r.json().then((data) => ({ ok: r.ok, data })))
      .then(({ ok, data }) => {
        if (!ok || !data.ok) {
          window.alert((data && data.error) || "Failed to remove the camera.");
          return;
        }
        window.location.href = "/";
      })
      .catch(() => window.alert("Failed to remove the camera."));
  });
  ...
  function showError(message) {
    ...
    errorBanner.textContent = "This camera failed to start: " + message + " -- fix its settings below and save to retry.";
  ...
  form.addEventListener("submit", (evt) => {
    ...
    fetch(API("/settings"), {
      ...
      .then((r) => r.json().then((data) => ({ ok: r.ok, data })))
      .then(({ ok, data }) => {
        statusEl.textContent = ok && data.ok ? "Saved." : "Error: " + (data.error || "failed to save settings.");
